...
public class GoodsDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mGoods.getImageDetail() != null && mGoods.getImageDetail().length() > 0) {
            banner_images.addAll(Arrays.asList(mGoods.getImageDetail().split(",")));
        } else if (mGoods.getImageIcon() != null && mGoods.getImageIcon().length() > 0) {
            banner_images.add(mGoods.getImageIcon());
        }
