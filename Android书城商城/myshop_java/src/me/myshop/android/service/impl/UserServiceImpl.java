...
//			}
    ...
    @Override
    public double recharge(Integer uid, double balance_change) {
        double balance_now = balance(uid);
        userMapper.updateBalanceById(uid, balance_now + balance_change);
        return balance(uid);
    }

    @Override
    public double balance(Integer uid) {
        User queryUser = userMapper.selectUserById(uid);
        return queryUser.getBalance();
    }
